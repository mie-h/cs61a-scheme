...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        #check_procedure(expr)
        proc = scheme_eval(first, env)
        check_procedure(proc)
        rest_proc = rest.map(lambda x: scheme_eval(x, env))
        return scheme_apply(proc, rest_proc, env)
        # END PROBLEM 4
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        return True
    while expressions.rest is not nil:
        if scheme_falsep(scheme_eval(expressions.first, env)) is True:
            return scheme_eval(expressions.first, env)
        expressions = expressions.rest
    return scheme_eval(expressions.first, env)
    # END PROBLEM 12
...
def do_or_form(expressions, env):
    ...
    while expressions is not nil:
        if scheme_truep(scheme_eval(expressions.first, env)) is True:
            return scheme_eval(expressions.first, env)
        expressions = expressions.rest
    return False
    # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        formals = nil
        expr = nil
        a_pair = bindings.first
        check_form(a_pair, 2, 2)
        evaluated_expr = eval_all(a_pair.rest, env)

        formals = Pair(a_pair.first, formals)
        expr = Pair(evaluated_expr, expr)

        check_formals(formals)
        bindings = bindings.rest
        new_env = new_env.make_child_frame(formals, expr)
